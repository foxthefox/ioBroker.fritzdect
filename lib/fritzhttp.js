...
class Fritz {
	...
	async setSwitchOn(ain) {
		const body = await Fritz.executeCommand(this.username, this.password, this.options, 'setswitchon', ain, '', '');
		return /^1/.test(body);
	}

	// turn an outlet off. returns the state the outlet was set to
	async setSwitchOff(ain) {
		const body = await Fritz.executeCommand(
			this.username,
			this.password,
			this.options,
			'setswitchoff',
	...
	static async check_SID(sid, options) {
		const body = await Fritz.executeCommand(null, null, null, null, options, '/login_sid.lua', sid);
