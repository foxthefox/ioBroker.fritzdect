...
class Fritz {
	...
	getDeviceListInfos() {
		console.log('func call getdevlist ' + JSON.stringify(this.options));
	...
	getTemplateListInfos() {
		console.log('func call gettempl ' + JSON.stringify(this.options));
	...
	static async executeCommand(username, password, url, command, ain, options, path) {
		...
		if (!path) {
			...
			console.log('path gesetzt ' + path);
		}
		console.log('path übernommen ' + path);
		let boxurl = url.url;
		let sid = await this.get_sid(boxurl, username, password, true);
		//console.log('execCMD sid ', sid, command);
		if (sid) {
			console.log('execCMD #2 ' + JSON.stringify(url));
			console.log('--------------------------');
			console.log('execCMD #2 ' + JSON.stringify(boxurl));
			if (sid) path += '&sid=' + sid;
