...
class Fritz {
	constructor(username, password, uri, strictssl) {
		this.sid = ''; // bringt nichts da hier derzeitig nichts gemerkt wird, gegenüber fritzapi wird in executeCMD die SID ermittelt und nicht übergeben
	...
	getDeviceListInfos() {
		return Fritz.executeCommand(this.username, this.password, this.options, 'getdevicelistinfos', '', '', ''); // aufruf einer statischen Klasse nur über this.constructor
	}
	// get template information (XML)
	getTemplateListInfos() {
		return Fritz.executeCommand(this.username, this.password, this.options, 'gettemplatelistinfos', '', '', '');
	}
	...
	getBasicDeviceStats(ain) {
		return Fritz.executeCommand(this.username, this.password, this.options, 'getbasicdevicestats', ain, '', '');
	}
	// get color defaults (XML)
	getColorDefaults() {
		return Fritz.executeCommand(this.username, this.password, this.options, 'getcolordefaults', '', '', '');
	}
	// check of permissions
	getUserPermissions() {
		return Fritz.getPermissions(this.username, this.password, this.options);
	}
	...
	setTempTarget(ain, temp) {
		return Fritz.executeCommand(
			...
			this.password,
			this.options,
			'sethkrtsoll&param=' + Fritz.temp2api(temp),
			ain,
			'',
			''
		).then(function(body) {
	...
	setWindowOpen(ain, time) {
		return Fritz.executeCommand(
			...
			this.password,
			this.options,
			'sethkrwindowopen&endtimestamp=' + time,
			ain,
			'',
			''
		).then(function(body) {
	...
	static calculate_pbkdf2_response(challenge, password) {
		...
		const salt1 = challenge_parts[2].toString('hex');
		const iter2 = Math.floor(challenge_parts[3]);
		const salt2 = challenge_parts[4].toString('hex');
		...
		const hash1 = crypto.pbkdf2Sync(password, salt1, iter1, 32, 'sha512');
		// py hash2 = hashlib.pbkdf2_hmac("sha256", hash1, salt2, iter2)
		const hash2 = crypto.pbkdf2Sync(hash1, salt2, iter2, 32, 'sha512');
	...
	static check_SID(sid, options) {
		return Fritz.executeCommand(null, null, null, null, options, '/login_sid.lua', sid).then(function(body) {
	...
	static async executeCommand(username, password, url, command, ain, options, path) {
		...
		const boxurl = url.url;
		const sid = await this.get_sid(boxurl, username, password, true); //instead of using url.url better to use the boxurl
	...
	static async getPermissions(username, password, url) {
		...
		//try
		const rights = await this.get_sid(url.url, username, password, false);
		if (rights) {
