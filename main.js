...
class Fritzdect extends utils.Adapter {
                        ...
                        if (idx.startsWith('DECT_')) {
                            ...
                            if (dp === 'tsoll') {
                                if (state.val < 8) {
                                    ...
                                    await this.setStateAsync(`DECT_${id}.hkrmode`, {
                                        val: 1,
                                        ack: false,
                                    }); //damit das Ventil auch regelt
                                    await this.fritz
                                    ...
                                    await this.setStateAsync(`DECT_${id}.hkrmode`, {
                                        val: 2,
                                        ack: false,
                                    }); //damit das Ventil auch regelt (false= Befehl und nochmaliger Einsprung )
                                    await this.fritz
                                        .setTempTarget(id, 'on')
                                        .then(() => {
                            ...
                            if (dp == 'boostactive') {
                                if (
                                    ...
                                    const minutes = await this.getStateAsync(`DECT_${id}.boostactivetime`).catch(
                                        error => {
                                    ...
                                    if (minutes && minutes.val !== null) {
                                        ...
                                        this.fritz
                                            ...
                                            .then(body => {
                                                ...
                                                this.log.debug(
                                                    `Set thermostat boost ${id} to ${state.val} until calculated ${
                                                        ende
                                                    } ${new Date(ende * 1000)}`,
                                                );
                                                this.setStateAsync(`DECT_${id}.boostactive`, {
                            ...
                            if (dp == 'windowopenactiv') {
                                if (
                                    ...
                                    const minutes = await this.getStateAsync(`DECT_${id}.windowopenactivetime`).catch(
                                        error => {
                                            this.log.warn(
                                    ...
                                    if (minutes && minutes.val !== null) {
                                        ...
                                        this.fritz
                                            ...
                                            .then(body => {
                                                ...
                                                this.log.debug(`window ${body} reading to ${endtime}`);
                                                this.log.debug(
                                                    `Set thermostat windowopen ${id} to ${state.val} until calculated ${
                                                        ende
                                                    } ${new Date(ende * 1000)}`,
                                                );
                                                this.setStateAsync(`DECT_${id}.windowopenactiv`, {
                            ...
                            if (dp == 'state') {
                                if (
                                    ...
                                    const switchtyp = await this.getStateAsync(`DECT_${id}.switchtype`).catch(error => {
                                    ...
                                    if (switchtyp && switchtyp.val !== null) {
                                        ...
                                        throw {
                                            error: 'could not determine the type of switch (switch/simpleonoff)',
                                        };
                                    }
                                } else if (
                                    state.val === 1 ||
                            ...
                            if (dp == 'blindsclose') {
                                this.fritz
                                    ...
                                    .then(async () => {
                                        ...
                                        await this.setStateAsync(`DECT_${id}.blindsclose`, {
                                            val: false,
                                            ack: true,
                                        }); //iobroker State-Bedienung wird nochmal als Status geschrieben, da API-Aufruf erfolgreich
                                    })
                            ...
                            if (dp == 'blindsopen') {
                                this.fritz
                                    ...
                                    .then(async () => {
                                        ...
                                        await this.setStateAsync(`DECT_${id}.blindsopen`, {
                                            val: false,
                                            ack: true,
                                        }); //iobroker State-Bedienung wird nochmal als Status geschrieben, da API-Aufruf erfolgreich
                                    })
                            ...
                            if (dp == 'blindsstop') {
                                this.fritz
                                    ...
                                    .then(() => {
                                        ...
                                        this.setStateAsync(`DECT_${id}.blindsstop`, {
                                            val: false,
                                            ack: true,
                                        }); //iobroker State-Bedienung wird nochmal als Status geschrieben, da API-Aufruf erfolgreich
                                    })
                            ...
                            if (dp == 'level') {
                                this.fritz
                                    ...
                                    .then(() => {
                                        ...
                                        this.setStateAsync(`DECT_${id}.level`, {
                                            val: state.val,
                                            ack: true,
                                        }); //iobroker State-Bedienung wird nochmal als Status geschrieben, da API-Aufruf erfolgreich
                                    })
                                ...
                                if (saturation && saturation.val !== null) {
                                    ...
                                    if (setSaturation == '') {
                                        ...
                                        this.fritz
                                            ...
                                            .then(() => {
                                                this.log.debug(
                                                    `Set lamp color hue ${id} to ${state.val} and saturation of ${
                                                        setSaturation
                                                    }`,
                                                );
                                ...
                                if (hue && hue.val !== null) {
                                    ...
                                    if (setHue == '') {
                                        ...
                                        this.fritz
                                            ...
                                            .then(() => {
                                                this.log.debug(
                                                    `Set lamp color saturation ${id} to ${state.val} and hue of ${
                                                        setHue
                                                    }`,
                                                );
                            ...
                            if (dp == 'unmapped_hue') {
                                const saturation = await this.getStateAsync(`DECT_${id}.unmapped_saturation`).catch(
                                    error => {
                                ...
                                if (
                                    ...
                                    this.fritz
                                        ...
                                        .then(sid => {
                                            ...
                                            this.setStateAsync('template.lasttemplate', {
                                                val: sid,
                                                ack: true,
                                            }); //when successfull toggle, the API returns the id of the template
                                        })
                                        .catch(e => this.errorHandlerApi(e));
                                }
                            }
                        ...
                        try {
                            ...
                            if (obj.callback) {
                                this.sendTo(
                                    ...
                                    {
                                        result: false,
                                        error: `unable to get manual updates${error}`,
                                    },
                                    obj.callback,
                                );
                            }
    ...
    /**
     * @param {any[]} devicearray - devicearray
     */
            ...
            if (deviceswithstat && deviceswithstat.val) {
                ...
                for (let i = 0; i < devstat.length; i++) {
                    ...
                    try {
                        ...
                        this.log.debug(`Error in processing stats. Device: ${devstat[i]}`);
                    ...
                    await Promise.all(
                        routines.map(async routine => {
                            ...
                            let old = await this.getStateAsync(
                                ...
                                this.log.warn(
                                    `problem getting routine_${routine.identifier.replace(/\s/g, '')}.active ${error}`,
                                );
                            });
                            if (old) {
                                ...
                                if (devices[i].identifier !== '' && devices[i].identifier) {
									...
									*/
                                        ...
                                        const obj = await this.getForeignObjectAsync(
                                            `${this.namespace}.DECT_${devices[i].identifier.replace(
                                                /\s/g,
                                                '',
                                            )}.present`,
                                        );
                                        if (!obj || !obj.common) {
                                            this.log.debug(
                                                `DECT_${devices[i].identifier.replace(
                                                    /\s/g,
                                                    '',
                                        ...
                                        if (devices[i].hkr) {
                                            ...
                                            let oldval = await this.getStateAsync(
                                                ...
                                                this.log.warn(
                                                    `problem getting DECT_${devices[i].identifier.replace(
                                                        /\s/g,
                                                        '',
                                                    )}.operationmode ${error}`,
                                                );
                    ...
                    await Promise.all(
                        groups.map(async device => {
                            ...
                            if (device.present === '0' || device.present === 0 || device.present === false) {
                                this.log.debug(
                                    `DECT_${device.identifier.replace(
                                        /\s/g,
                                        '',
                                    )} is not present, check the device connection, no values are written`,
                                );
                    ...
                    if (key == 'energy') {
                        ...
                        if (obj['stats']) {
                            if (obj['stats'][0]) {
                                ...
                                old = await this.getStateAsync(`DECT_${identifier}.${key}_stats.countm`).catch(
                                    error => {
                                ...
                                old = await this.getStateAsync(`DECT_${identifier}.${key}_stats.gridm`).catch(error => {
                                ...
                                let datatimem = obj['stats']['datatime']
                                    ? parseInt(obj['stats'][0]['datatime'])
                                    : Date.now();
                                ...
                                let monthnum = obj['stats']['datatime']
                                    ? parseInt(new Date(datatimem * 1000).toISOString().slice(5, 7)) // *1000 entfernen?
                                    : Date.now();
                                ...
                                // days
                                if (obj['stats'][1]) {
                                    old = await this.getStateAsync(`DECT_${identifier}.${key}_stats.countd`).catch(
                                        error => {
                                    ...
                                    old = await this.getStateAsync(`DECT_${identifier}.${key}_stats.gridd`).catch(
                                        error => {
                                    ...
                                    let datatimed = obj['stats']['datatime']
                                        ? parseInt(obj['stats'][1]['datatime'])
                                        : Date.now();
                        ...
                        if (obj['stats']) {
                            old = await this.getStateAsync(`DECT_${identifier}.${key}_stats.count`).catch(error => {
                            ...
                            old = await this.getStateAsync(`DECT_${identifier}.${key}_stats.grid`).catch(error => {
                ...
                try {
                    ...
                    if (key == 'nextchange') {
                        ...
                        if (changed || !this.config.fritz_writeonhyst) {
                            this.log.debug(`updating data DECT_${ain} : ${key} new: ${batt} old: ${old.val}`);
                            await this.setStateAsync(`DECT_${ain}.${key}`, {
                                val: batt,
                                ack: true,
                            });
                        ...
                        if (changed || !this.config.fritz_writeonhyst) {
                            this.log.debug(
                                `updating data DECT_${ain} : ${key} new: ${parseFloat(value) / 10} old: ${old.val}`,
                            );
                            await this.setStateAsync(`DECT_${ain}.${key}`, {
                        ...
                        if (changed || !this.config.fritz_writeonhyst) {
                            this.log.debug(
                                `updating data DECT_${ain} : ${key} new: ${parseFloat(value) / 1000} old: ${old.val}`,
                            );
                            await this.setStateAsync(`DECT_${ain}.${key}`, {
                        ...
                        if (value < 70) {
                            ...
                            if (changed || !this.config.fritz_writeonhyst) {
                                this.log.debug(`updating data DECT_${ain} : ${key} new: ${newtemp} old: ${old.val}`);
                                await this.setStateAsync(`DECT_${ain}.${key}`, {
                                    val: newtemp,
                                    ack: true,
                                });
                            ...
                            if (!isNaN(old.val)) {
                                ...
                                // skip new setting
                                this.log.debug(`no update data DECT_${ain} : ${key} new: ${newtemp} old: ${old.val}`);
                            }
                        }
                        ...
                        if (changed || !this.config.fritz_writeonhyst) {
                            this.log.debug(
                                `updating data DECT_${ain} : ${key} new: ${parseFloat(value)} old: ${old.val}`,
                            );
                            await this.setStateAsync(`DECT_${ain}.${key}`, {
                        ...
                        if (changed || !this.config.fritz_writeonhyst) {
                            this.log.debug(`updating data DECT_${ain} : ${key} new: ${convertValue} old: ${old.val}`);
                            await this.setStateAsync(`DECT_${ain}.${key}`, {
                                val: convertValue,
                                ack: true,
                            });
                        ...
                        if (changed || !this.config.fritz_writeonhyst) {
                            this.log.debug(`updating data DECT_${ain} : ${key} new: ${convTime} old: ${old.val}`);
                            await this.setStateAsync(`DECT_${ain}.${key}`, {
                                val: convTime, //Str()
                                ack: true,
                            });
        ...
        for (let device of devices) {
            if (device.identifier !== '' && device.identifier) {
                ...
                if ((device.functionbitmask & 64) == 64) {
                    ...
                    (device.functionbitmask & 512) == 512 ||
                    (device.functionbitmask & 128) == 128
                ) {
                    //DECT200/210
                ...
                this.log.debug(
                    `device ${identifier} named ${device.name} mask ${device.functionbitmask} assigned to ${role}`,
                );
                if (role != 'etsi') {
                        ...
                        await Promise.all(
                            Object.keys(device.switch).map(async key => {
                                ...
                                if (key === 'state') {
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.state`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                            if (obj) {
                                                obj.common.role = 'switch.light'; // modify object
                                                this.extendForeignObject(obj._id, obj, err => {
                                                    if (err) {
                                                        this.log.error(String(err));
                                                    }
                                                });
                        ...
                        await Promise.all(
                            Object.keys(device.hkr).map(async key => {
                                ...
                                if (key === 'tist') {
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.tsoll`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                            if (obj) {
                                                obj.common.role = 'level.temperature'; // modify object
                                                this.extendForeignObject(obj._id, obj, err => {
                                                    if (err) {
                                                        this.log.error(String(err));
                                                    }
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.boostactive`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.boostactivetime`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                            if (obj) {
                                                obj.common.role = 'level'; // modify object
                                                this.extendForeignObject(obj._id, obj, err => {
                                                    if (err) {
                                                        this.log.error(String(err));
                                                    }
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.windowopenactiv`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.windowopenactivetime`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                        ...
                        await Promise.all(
                            Object.keys(device.levelcontrol).map(async key => {
                                ...
                                if (key === 'level') {
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.level`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.levelpercentage`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                        ...
                        await Promise.all(
                            Object.keys(device.colorcontrol).map(async key => {
                                ...
                                if (key === 'supported_modes') {
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.hue`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                            if (obj) {
                                                obj.common.role = 'level.color.hue'; // modify object
                                                this.extendForeignObject(obj._id, obj, err => {
                                                    if (err) {
                                                        this.log.error(String(err));
                                                    }
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.saturation`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                            if (obj) {
                                                obj.common.role = 'level.color.saturation'; // modify object
                                                this.extendForeignObject(obj._id, obj, err => {
                                                    if (err) {
                                                        this.log.error(String(err));
                                                    }
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.unmapped_hue`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.unmapped_saturation`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                    ...
                                    //state role change
                                    this.getObject(`DECT_${identifier}.temperature`, (err, obj) => {
                                        if (err) {
                                            this.log.error(String(err));
                                        } else {
                                            this.log.info(JSON.stringify(obj));
                                            if (obj) {
                                                obj.common.role = 'level.color.temperature'; // modify object
                                                this.extendForeignObject(obj._id, obj, err => {
                                                    if (err) {
                                                        this.log.error(String(err));
                                                    }
    ...
    async createTemplate(typ, newId, name, role, id) {
        ...
        await this.setStateAsync(`${typ + newId}.toggle`, {
            val: false,
            ack: true,
        });
        return;
